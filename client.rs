    ...
    match timeout(TIMEOUT_DURATION, socket.recv_from(&mut buffer)).await {
        Ok(Ok((len, addr))) => {
            ...
            if response.starts_with("LEADER,") {
                ...
                // Step 3: Loop to send multiple requests for image encryption and decryption
                for i in 1..=3 {
                    println!("Sending request #{} to leader for image transfer and encryption.", i);
                    middleware_encrypt(&socket, leader_address, "original_image.jpg".to_string(), i).await?;
        ...
        println!("Time taken to run main function: {:?}", duration);
    Ok(())
}
...
async fn middleware_decrypt(socket: &UdpSocket, request_id: u32) -> Result<(), Box<dyn Error>> {
    // Receive the encrypted image from the leader, using a unique filename per request
    let encoded_image_path = format!("encoded_image_received_{}.png", request_id);
    receive_image(socket, &encoded_image_path).await?;

    // Decode the received image and save each decoded output with a unique filename
    let decoded_output_path = format!("decoded_output_{}.png", request_id);
    decode_image(&encoded_image_path, &decoded_output_path).await?;
    Ok(())
}

async fn receive_image(socket: &UdpSocket, file_path: &str) -> tokio::io::Result<()> {
    ...
    println!("Client: Image successfully received and saved as {}.", file_path);
    Ok(())
}
/// Decode the received image and save the decoded output.
async fn decode_image(encoded_image_path: &str, decoded_output_path: &str) -> Result<(), Box<dyn Error>> {
    let encoded_image = file_as_dynamic_image(encoded_image_path.to_string()).to_rgba();
    let decoder = Decoder::new(encoded_image);

    let decoded_data = decoder.decode_alpha();
    std::fs::write(decoded_output_path, &decoded_data)?;
    println!("Image decoded and saved to {}", decoded_output_path);
    Ok(())
}
