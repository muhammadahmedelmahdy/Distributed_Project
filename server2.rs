...
// const SERVER_ADDR: &str = "172.18.0.1:8081"; // Update this per server instance
...
//     let mut file = File::create("received_image_2.jpg").await?;
...
//     let received_image_path = "received_image_2.jpg";
//     let mask_image_path = "mask.jpeg";
//     let encoded_image_path = "encoded_received_image_2.png";
...
const SERVER_ADDR: &str = "10.40.49.222:8081";
...
fn determine_cpu_utilization() -> f32 {
...
fn choose_leader_per_request() {
    ...
    let cpu_utilization = determine_cpu_utilization();
    let leader_index = rand::thread_rng().gen_range(0..SERVER_ADDRS.len());
    
    // Logic to choose leader based on CPU utilization
    if cpu_utilization < 50.0 && SERVER_ADDR == SERVER_ADDRS[leader_index] {
        IS_LEADER.store(true, Ordering::SeqCst);
        println!("Server {}: Elected as the leader for this request (CPU usage: {:.2}%).", SERVER_ADDR, cpu_utilization);
    } else {
        IS_LEADER.store(false, Ordering::SeqCst);
        println!("Server {}: Not elected as the leader for this request (CPU usage: {:.2}%).", SERVER_ADDR, cpu_utilization);
    }
...
async fn encode_received_image() -> tokio::io::Result<()> {
    ...
    let mask_image_path = "mask.jpeg";
    let encoded_image_path = "encrypted_image_2.png";
